...
public class LockService {
    ...
    public LockService(RedisLockRegistry lockRegistry, JdbcLockRegistry jdbcLockRegistry) {
        this.redisLockRegistry = lockRegistry;
        this.jdbcLockRegistry = jdbcLockRegistry;
    }

    public String redisLock(Time wait){
        var actions = createActions("Redis");
        return lock(redisLockRegistry, actions, wait);
    }

    public String jdbcLock(Time wait){
        var actions = createActions("JDBC");
        return lock(jdbcLockRegistry, actions, wait);
    }
    ...
    public String testWaitLock(long lockWaitSeconds, long threadSleepSeconds) {
        ...
        var threadOne = createTestThread(jdbcLockRegistry, waitTime, threadSleepSeconds);
        var threadTwo = createTestThread(jdbcLockRegistry, waitTime, threadSleepSeconds);
    ...
    private record Time(long duration, TimeUnit timeUnit){ }

    private record LockableActions<T>(Supplier<T> onSuccess, Supplier<T> onFailure,
